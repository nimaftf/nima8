        ...
        if cmd == "silent" then
            local function silent_cb(arg, data)
                ...
                if group[tostring(arg.chat_id)]['is_silent_users'][tostring(data.id_)] then
                    tg.sendMessage(arg.chat_id, "", 0, "[`"..data.id_.."`] *is already mute*", 0, "md")
                end
                group[tostring(arg.chat_id)]['is_silent_users'][tostring(data.id_)] = user_name
                save_data(_config.group.data, group)
                tg.sendMessage(arg.chat_id, "", 0, "[`"..data.id_.."`] *added to mute users list*", 0, "md")
            end
            tdcli_function ({
            ID = "GetUser",
        ...
        if cmd == "unsilent" then
            local function unsilent_cb(arg, data)
                ...
                if not group[tostring(arg.chat_id)]['is_silent_users'][tostring(data.id_)] then
                    tg.sendMessage(arg.chat_id, "", 0, "[`"..data.id_.."`] *is not mute*", 0, "md")
                end
                group[tostring(arg.chat_id)]['is_silent_users'][tostring(data.id_)] = nil
                save_data(_config.group.data, group)
                tg.sendMessage(arg.chat_id, "", 0, "[`"..data.id_.."`] *removed from mute users list*", 0, "md")
            end
            tdcli_function ({
            ID = "GetUser",
            user_id_ = data.sender_user_id_
        ...
        if matches[1] == "muteuser" then
			...
			if matches[2] and string.match(matches[2], '^%d+$') then
				...
				if is_silent_user(matches[2], chat) then
					tg.sendMessage(msg.chat_id_, "", 0, "[`"..matches[2].."`] *is already mute*", 0, "md")
				end
				group[tostring(chat)]['is_silent_users'][tostring(matches[2])] = ""
				save_data(_config.group.data, group)
				tg.sendMessage(msg.chat_id_, msg.id_, 0, "[`"..matches[2].."`] *added to mute users list*", 0, "md")
			end
		...
		if matches[1] == "unmuteuser" then
			...
			if matches[2] and string.match(matches[2], '^%d+$') then
				if not is_silent_user(matches[2], chat) then
					tg.sendMessage(msg.chat_id_, "", 0, "[`"..matches[2].."`] *is not mute*", 0, "md")
				end
				group[tostring(chat)]['is_silent_users'][tostring(matches[2])] = nil
				save_data(_config.group.data, group)
				tg.sendMessage(msg.chat_id_, msg.id_, 0, "[`"..matches[2].."`] *removed from mute users list*", 0, "md")
			end
			if matches[2] and not string.match(matches[2], '^%d+$') then
				tdcli_function ({
				ID = "SearchPublicChat",
