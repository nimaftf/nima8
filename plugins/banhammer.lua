local function run(msg ,matches)
	...
	elseif msg.text:match("@(.*)") then
		...
		tg.changeChatMemberStatus(msg.chat_id_, msg.text:match("@(.*)"), 'Kicked')	
	...
	elseif msg.text:match("@(.*)") then
		...
		tg.changeChatMemberStatus(msg.chat_id_, msg.text:match("@(.*)"), 'Banned')	
	elseif tonumber(matches[2]) then
         tg.sendMessage(msg.chat_id, 0, 0,  '[ '..matches[2]..' ] Banned', 0)
		tg.changeChatMemberStatus(msg.chat_id_, data.id_, 'Banned')	
	end
	end
end
return {
